\documentclass[12pt, a4paper, oneside]{book} %12pt is formal, 17pt is for extbook
\usepackage[utf8]{inputenc}
\usepackage{minitoc}
%% Figure settings
% -----------------------------------------
\ifx\pdfoutput\undefined
%Use old Latex if PDFLatex does not work
\usepackage[dvips]{graphicx}% To get graphics working
\usepackage{float} % in order to use H for figure position
\DeclareGraphicsExtensions{.eps} % Encapsulated PostScript
\else
%Use PDFLatex
\usepackage[pdftex]{graphicx}% To get graphics working
\DeclareGraphicsExtensions{.pdf,.jpg,.png,.mps} % Portable Document Format, Joint Photographic Experts Group, Portable Network Graphics, MetaPost
\pdfcompresslevel=9
\fi
\usepackage{pgffor}
\usepackage{caption}
\usepackage{subcaption} % maybe for caption*
\graphicspath{{../Figures/}}
% -----------------------------------------
%%
\usepackage[table,xcdraw]{xcolor}
\usepackage{rotating}
\usepackage{tabulary}
\usepackage{longtable}
\usepackage{lscape}
\usepackage{multirow}
\usepackage{multicol}
\usepackage{makecell}
\usepackage[titletoc]{appendix}
\usepackage{csquotes}
\usepackage{lineno} % line numbers Butters
%% Font style setting
% -----------------------------------------
\usepackage{mathspec}
\setmainfont{Times New Roman}
\setmathsfont{Times New Roman}
\usepackage{xeCJK}
\setCJKmainfont{標楷體}
\XeTeXlinebreaklocale "zh"
\XeTeXlinebreakskip = 0pt plus 1pt
\defaultCJKfontfeatures{AutoFakeBold=6,AutoFakeSlant=.4}
% -----------------------------------------
%%
% -----------------------------------------
%%

% -----------------------------------------
%% Chinese Font bf setting
% -----------------------------------------
\def\xeCJKembold{0.4}

% hack into xeCJK, you don't need to understand it
\def\saveCJKnode{\dimen255\lastkern}
\def\restoreCJKnode{\kern-\dimen255\kern\dimen255}

% save old definition of \CJKsymbol and \CJKpunctsymbol for CJK output
\let\CJKoldsymbol\CJKsymbol
\let\CJKoldpunctsymbol\CJKpunctsymbol

% apply pdf literal fake bold
\def\CJKfakeboldsymbol#1{%
	\special{pdf:literal direct 2 Tr \xeCJKembold\space w}%
	\CJKoldsymbol{#1}%
	\saveCJKnode
	\special{pdf:literal direct 0 Tr}%
	\restoreCJKnode}
\def\CJKfakeboldpunctsymbol#1{%
	\special{pdf:literal direct 2 Tr \xeCJKembold\space w}%
	\CJKoldpunctsymbol{#1}%
	\saveCJKnode
	\special{pdf:literal direct 0 Tr}%
	\restoreCJKnode}
\newcommand\CJKfakebold[1]{%
	\let\CJKsymbol\CJKfakeboldsymbol
	\let\CJKpunctsymbol\CJKfakeboldpunctsymbol
	#1%
	\let\CJKsymbol\CJKoldsymbol
	\let\CJKpunctsymbol\CJKoldpunctsymbol}
%%
\usepackage{fontspec}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\usepackage{wallpaper}
\usepackage{setspace}
\usepackage{authblk}
\usepackage{amsmath,amssymb,bm}
\allowdisplaybreaks
\usepackage[english]{babel}

% \usepackage{natbib}
% \bibliographystyle{apalike}

\usepackage[square,numbers]{natbib} %Nice numbered citations
\bibliographystyle{unsrtnat}

% \usepackage[american]{babel}
% \usepackage{csquotes}
% \usepackage[style=mla]{biblatex}

\usepackage[hidelinks]{hyperref}
\usepackage{breakcites}
\usepackage{../macros_tn} %Loads all my macros
\usepackage{geometry}

% \usepackage[colorinlistoftodos, textwidth=40mm, textsize=small]{todonotes}   % notes showed
\usepackage[disable]{todonotes} % notes disable

% \usepackage[disable]{todonotes} % hide all todos
\usepackage{enumitem}
\usepackage{indentfirst}
%% Date setting for cover
% -----------------------------------------
\usepackage{datetime} % For displaying month and year in the cover
\usepackage{fp} % To calculate the year of Minguo(民國)
\FPsub\Minguo{\the\year}{1911}
\FPeval{\Minguo}{round(Minguo,0)}
% -----------------------------------------
%%
%% Nomenclature
% -----------------------------------------
\usepackage{nomencl}
\makenomenclature
% -----------------------------------------
%%
%% Grouping nomenclature
% -----------------------------------------
\usepackage{etoolbox}
\renewcommand\nomgroup[1]{%
	\item[\bfseries
			\ifstrequal{#1}{A}{List of abbreviations}{
					\ifstrequal{#1}{B}{List of symbols}{
						% \ifstrequal{#1}{C}{Attention Mechanism} {
						% }
					}
			}
		]
}
% -----------------------------------------
%%
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{1} % Index depth Butters
\usepackage{lipsum}

\hypersetup{
  colorlinks=true,
  linkcolor=black, 
  citecolor=blue,
  urlcolor=magenta,
  filecolor=cyan,
  linkbordercolor={0 0 1}
}

%% checkmark
\usepackage{amssymb}% http://ctan.org/pkg/amssymb
\usepackage{pifont}% http://ctan.org/pkg/pifont
\newcommand{\cmark}{\ding{51}}%
\newcommand{\xmark}{\ding{55}}%

%%%%%%%%%%%%%%%%%%%%%%% the page numbers location 
% \pagestyle{headings}
%%%%%%%%%%%%%%%%%%%%%%%
% \pagestyle{plain} % good but no header
%%%%%%%%%%%%%%%%%%%%%%%%%%

\usepackage{fancyhdr}
\pagestyle{fancy}
% \fancyhf{}
% \rhead{}
\lhead{  }
\cfoot{\thepage}
% \rfoot{doi:10.6844/NCKU202100120}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Please create a new tex file and copy the content of this template%
%                    START WRITING FROM HERE                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}
	% \linenumbers     % show line numbers Butters
	\pagenumbering{gobble}
	\begin{titlepage}
		\newgeometry{a4paper,
		total={210mm,296mm},
		top=23mm,
		bottom=30mm,
		left=20mm,
		right=20mm,}
	\input{../Cover.tex}
	\ThisCenterWallPaper{.325}{watermark.jpg}
	\end{titlepage}
	\newgeometry{
		a4paper,
		total={210mm,296mm},
		left = 30mm, %30, 10
		right = 25mm, %25, 45
		top = 23mm,  %23, 15
		bottom = 35mm,  %35, 15
	}
	\CenterWallPaper{.325}{watermark.jpg} % disable watermark Butters
	\begin{spacing}{1.5} %1.5
		\frontmatter
		%% Chinese Abstract
		\addcontentsline{toc}{chapter}{Chinese abstract}
		\chapter*{\centering Chinese abstract}
		\label{chap:chi_abstract}
		\noindent % Butters
		\input{../Chinese_abstract.tex}
		\newpage
		%% Abstract
		\addcontentsline{toc}{chapter}{Abstract}
		\chapter*{\centering Abstract}
		\label{chap:abstract}
		\noindent % Butters
		\input{../Abstract.tex}
		\newpage
		%% acknowledgment
		\chapter*{\centering Acknowledgment}
		\addcontentsline{toc}{chapter}{Acknowledgment}
		\label{chap:acknowledgment}
		\input{../Acknowledgment.tex}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Editing_notes start
		% \newpage
		% \chapter*{\centering Editing notes}
		% \addcontentsline{toc}{chapter}{Editing notes}
		% \label{chap:Editing_notes}
		% \input{../Editing_notes.tex}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Editing_notes end
		\newpage
		%% Table of contents
		\dominitoc 
		\begin{singlespace}
		\renewcommand{\contentsname}{Table of Contents}
		\cleardoublepage
		\tableofcontents
		\addcontentsline{toc}{chapter}{Table of Contents}

		%% List of tables		
		\listoftables
		\addcontentsline{toc}{chapter}{List of Tables}

		%% List of figures		
		\listoffigures
		\addcontentsline{toc}{chapter}{List of Figures}

		%% List of symbols
		% Note: add the following setting in configure of makeindex
		% makeindex.exe %.nlo -s nomencl.ist -o %.nls -t %.nlg
		\mbox{}
		%% ---- Start writing nomenclature here ---- %%
	
		\input{../Nomenclature.tex}
		\printnomenclature
		\addcontentsline{toc}{chapter}{Nomenclature}
		
		\end{singlespace}
		%% Main content
		\mainmatter
		\chapter{Introduction}
		\label{chap:Introduction}
		\input{../Introduction.tex}
		\chapter{Related Works}
		\label{chap:Related_work}
		\input{../Related_work.tex}
		\chapter{Neural Decoder Design}
		\label{chap:Neural_decoder_design}
		\input{../Neural_decoder_design.tex}
		\chapter{Experimental Results}
		\label{chap:Experimental_results}
		\input{../Experimental_results.tex}
		\chapter{Discussion}
		\label{chap:Discussion}
		\input{../Discussion.tex}
		\chapter{Conclusions}
		\label{chap:Conclusion}
		\input{../Conclusion.tex}
		\begin{singlespace}
			\clearpage
			%% References (bibliography)

			\renewcommand{\bibname}{References}

			%% ----  Bib file here ---- %%
			\addcontentsline{toc}{chapter}{References}
			\bibliography{../reference}
			\clearpage
		\end{singlespace}
		%% Appendix
		\begin{appendices}
			\chapter{Monkey Indy}
			\input{../Appendix_indy.tex}
			\chapter{Monkey Loco}
			\input{../Appendix_loco.tex}
			% \chapter{Monkey Chewie}
			% \input{../Appendix_chewie.tex}
		\end{appendices}
		\backmatter
	\end{spacing}
\end{document}      
