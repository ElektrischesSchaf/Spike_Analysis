...
class  LSTMModel(torch.nn.Module):
    ...
    def __init__(self, firing_rate_input_dim, result_conv_input_dim, hidden_dim, layer_dim, output_dim):
        ...
        self.lstm_spike = torch.nn.LSTM(firing_rate_input_dim, hidden_dim, layer_dim, batch_first=True, bidirectional=False)
        self.lstm_conv_average_phase = torch.nn.LSTM(result_conv_input_dim, hidden_dim, layer_dim, batch_first=True, bidirectional=False)
        self.lstm_conv_average_sync = torch.nn.LSTM(result_conv_input_dim, hidden_dim, layer_dim, batch_first=True, bidirectional=False)
    ...
    def forward(self, x):
        ...
        # time steps
        out_spike, _ = self.lstm_spike(x[ : , : , : self.firing_rate_input_dim ])
        out_conv_average_phase, _ = self.lstm_conv_average_phase(x[ : , : , self.firing_rate_input_dim : self.firing_rate_input_dim+self.result_conv_input_dim ])
        out_conv_average_sync, _ = self.lstm_conv_average_sync(x[ : , : , self.firing_rate_input_dim+self.result_conv_input_dim : self.firing_rate_input_dim+self.result_conv_input_dim+self.result_conv_input_dim ])
