...
class Real_Layer_GRU_one_way_parallel_3(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, max_timestep, layer_dim, output_dim):
        super(Real_Layer_GRU_one_way_parallel_3, self).__init__()
        # Hidden dimensions
        self.hidden_dim = hidden_dim
        self.input_dim=input_dim
        # Number of hidden layers
        ...
        self.temporal_attention_feature_LN = torch.nn.LayerNorm([ r_temporal*hidden_dim ])
        self.unit_attention_feature_LN = torch.nn.LayerNorm([ r_hidden_units*max_timestep ])

        self.merged_feature_for_x_1 = torch.nn.Linear( int( r_temporal*hidden_dim+r_hidden_units*max_timestep ) , int(( r_temporal*hidden_dim+r_hidden_units*max_timestep )/2) )
        self.merged_feature_for_y_1 = torch.nn.Linear( int( r_temporal*hidden_dim+r_hidden_units*max_timestep ) , int(( r_temporal*hidden_dim+r_hidden_units*max_timestep )/2) )

        self.merged_feature_for_x_2 = torch.nn.Linear( int(( r_temporal*hidden_dim+r_hidden_units*max_timestep )/2) , 1 )
        self.merged_feature_for_y_2 = torch.nn.Linear( int(( r_temporal*hidden_dim+r_hidden_units*max_timestep )/2) , 1 )
    ...
    def forward(self, x):
        ...
        hidden_matrix_temporal = hidden_matrix_temporal.view( -1, hidden_matrix_temporal.size()[1]*hidden_matrix_temporal.size()[2] )
        hidden_matrix_units = hidden_matrix_units.view( -1, hidden_matrix_units.size()[1]*hidden_matrix_units.size()[2] )

        cancate_feature = torch.cat( ( self.temporal_attention_feature_LN(hidden_matrix_temporal) , self.unit_attention_feature_LN(hidden_matrix_units) )  ,1 )
        # cancate_feature = torch.cat( (hidden_matrix_temporal , hidden_matrix_units )  ,1 )

        cancate_for_x = cancate_feature.clone()
        cancate_for_y = cancate_feature.clone()

        cancate_for_x = torch.relu( self.merged_feature_for_x_1(cancate_for_x) )
        cancate_for_y = torch.relu( self.merged_feature_for_y_1(cancate_for_y) )

        cancate_for_x = self.merged_feature_for_x_2(cancate_for_x)
        cancate_for_y = self.merged_feature_for_y_2(cancate_for_y)


        out = torch.cat(  (cancate_for_x, cancate_for_y),1 )
...
class  Real_Layer_GRU_one_way_sequential_1(torch.nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, max_timestep, layer_dim, output_dim):
        super(Real_Layer_GRU_one_way_sequential_1, self).__init__()
        # Hidden dimensions
        self.hidden_dim = hidden_dim
        self.input_dim=input_dim
        # Number of hidden layers
        ...
        da= int( hidden_dim/2 )
        r =  max_timestep # int(max_timestep/2)

        da_hidden_units = int( hidden_dim/2 )
        r_hidden_units = int( hidden_dim/1 )

        self.W_s1_1 = torch.nn.Linear( hidden_dim, da )
        self.W_s2_1 = torch.nn.Linear( da, r )

        self.W_s1_2 = torch.nn.Linear( r, da_hidden_units )
        self.W_s2_2 = torch.nn.Linear( da_hidden_units, r_hidden_units )

        self.fc_layer_1_x = torch.nn.Linear( r*r_hidden_units, int(r*r_hidden_units/2))
        self.fc_layer_1_y = torch.nn.Linear( r*r_hidden_units, int(r*r_hidden_units/2))

        self.label_x = torch.nn.Linear( int(r*r_hidden_units/2) , 1 )
        self.label_y = torch.nn.Linear( int(r*r_hidden_units/2) , 1 )
    ...
    def attention_net_hidden_units( self, last_output ):
        attn_weight_matrix = self.W_s2_2(torch.tanh(self.W_s1_2(last_output)))
        attn_weight_matrix = attn_weight_matrix.permute(0, 2, 1)
    ...
    def forward(self, x):
        ...
        hidden_matrix_temporal = hidden_matrix_temporal.permute(0, 2, 1)
        # print('size of hidden_matrix_temporal: ', hidden_matrix_temporal.size(), '\n' )
        
        attn_weight_matrix_units = self.attention_net_hidden_units( hidden_matrix_temporal )
        hidden_matrix = torch.bmm( attn_weight_matrix_units, hidden_matrix_temporal )

        hidden_matrix_x = hidden_matrix.clone()
        hidden_matrix_y = hidden_matrix.clone()

        out_x = torch.relu( self.fc_layer_1_x( hidden_matrix_x.view( -1 , hidden_matrix_x.size()[1]*hidden_matrix_x.size()[2] ) ) )
        out_y = torch.relu( self.fc_layer_1_y( hidden_matrix_y.view( -1 , hidden_matrix_y.size()[1]*hidden_matrix_y.size()[2] ) ) )
...
class  Real_Layer_GRU_one_way_sequential_2(torch.nn.Module):

    def __init__(self, input_dim, hidden_dim, max_timestep, layer_dim, output_dim):
        super(Real_Layer_GRU_one_way_sequential_2, self).__init__()
        # Hidden dimensions
        self.hidden_dim = hidden_dim
        self.input_dim=input_dim
        # Number of hidden layers
        ...
        # Layer Normalization
        self.LN_temporal = torch.nn.LayerNorm( [ int(r*hidden_dim) ], elementwise_affine = False )
        self.LN_units = torch.nn.LayerNorm( [ int(r*r_hidden_units) ], elementwise_affine = False )
        

        self.fc_layer_1_x = torch.nn.Linear( int(r*r_hidden_units)+int(r*hidden_dim) , int(r*r_hidden_units/2))
        self.fc_layer_1_y = torch.nn.Linear( int(r*r_hidden_units)+int(r*hidden_dim) , int(r*r_hidden_units/2))
    ...
    def forward(self, x):
        ...
        hidden_matrix_temporal_cloned = hidden_matrix_temporal.permute(0, 2, 1).clone().detach_()
        # print('size of hidden_matrix_temporal_cloned: ', hidden_matrix_temporal_cloned.size(), '\n' )
        
        attn_weight_matrix_units = self.attention_net_hidden_units( hidden_matrix_temporal_cloned )
        hidden_matrix = torch.bmm( attn_weight_matrix_units, hidden_matrix_temporal_cloned )

        hidden_matrix_x = hidden_matrix.clone()
        hidden_matrix_y = hidden_matrix.clone()

        flatten_x_1 = hidden_matrix_temporal.clone().view( -1 , hidden_matrix_temporal.size()[1]*hidden_matrix_temporal.size()[2] )
        flatten_y_1 = hidden_matrix_temporal.clone().view( -1 , hidden_matrix_temporal.size()[1]*hidden_matrix_temporal.size()[2] )

        flatten_x_2 = hidden_matrix_x.view( -1 , hidden_matrix_x.size()[1]*hidden_matrix_x.size()[2] )
        flatten_y_2 = hidden_matrix_y.view( -1 , hidden_matrix_y.size()[1]*hidden_matrix_y.size()[2] )

        flatten_x_1=self.LN_temporal(flatten_x_1)
        flatten_y_1=self.LN_temporal(flatten_y_1)

        flatten_x_2=self.LN_units(flatten_x_2)
        flatten_y_2=self.LN_units(flatten_y_2)

        out_x = torch.relu( self.fc_layer_1_x(  torch.cat((flatten_x_1, flatten_x_2), 1)  ))
        out_y = torch.relu( self.fc_layer_1_y(  torch.cat((flatten_y_1, flatten_y_2), 1)  ))
...
class  Real_Layer_GRU_one_way_sequential_3(torch.nn.Module):

    def __init__(self, input_dim, hidden_dim, max_timestep, layer_dim, output_dim):
        super(Real_Layer_GRU_one_way_sequential_3, self).__init__()
        # Hidden dimensions
        self.hidden_dim = hidden_dim
        self.input_dim=input_dim
        # Number of hidden layers
        ...
        da_temporal = int( hidden_dim/2 )
        r_temporal  = int( hidden_dim/1 )

        da_hidden_units = int( max_timestep/2 )
        r_hidden_units = int( max_timestep/1 )

        self.GRU_Cell_forward_1 = LayerNormGRUCell( r_hidden_units      , hidden_dim    ,  bias=True )
        self.GRU_Cell_forward_2 = LayerNormGRUCell( hidden_dim      , hidden_dim    ,  bias=True )
        ...
        self.fc_layer_x = torch.nn.Linear( int(r_temporal*hidden_dim), int((r_temporal*hidden_dim)/2)  )
        self.fc_layer_y = torch.nn.Linear( int(r_temporal*hidden_dim), int((r_temporal*hidden_dim)/2)  ) 

        self.label_x = torch.nn.Linear( int((r_temporal*hidden_dim)/2) , 1 )
        self.label_y = torch.nn.Linear( int((r_temporal*hidden_dim)/2) , 1 )
    ...
    def forward(self, x):
        ...
        attn_weight_matrix_units = self.attention_net_hidden_units( x )

        # print('shape of attn_weight_matrix_units= ', attn_weight_matrix_units.size(),  '\n')

        hidden_matrix_units = torch.bmm( attn_weight_matrix_units, x )

        hidden_matrix_units=hidden_matrix_units.permute(0,2,1)
        # print('shape of hidden_matrix_units= ', hidden_matrix_units.size(),  '\n')

        # Initialize hidden state with zeros
        h0 = torch.zeros( hidden_matrix_units.size(0), self.hidden_dim).requires_grad_() # one-directional
        h0=h0.to(device)

        hidden_state_list=[]
        out_list=[]
        # time steps
        for i , input_t in enumerate( hidden_matrix_units.chunk( hidden_matrix_units.size(1), dim=1 )):
            input_t = input_t.squeeze(1)
            # print('shape of input_t= ', input_t.size(), '\n')
            h0 = self.GRU_Cell_forward_1(input_t, h0)
            hidden_state_list += [h0]
        ...
        hidden_matrix_x = hidden_matrix_temporal.clone()
        hidden_matrix_y = hidden_matrix_temporal.clone()

        flatten_x = hidden_matrix_x.view( -1 , hidden_matrix_x.size()[1]*hidden_matrix_x.size()[2] )
        flatten_y = hidden_matrix_y.view( -1 , hidden_matrix_y.size()[1]*hidden_matrix_y.size()[2] )

        out_x = torch.relu( self.fc_layer_x( flatten_x ))
        out_y = torch.relu( self.fc_layer_y( flatten_y ))
