...
class regular_modules():
    ...
    def evlauate_performance( self, real_y, predicted_y, max_timestep ):
        ...
        testing_data_RMSE = np.sqrt(mean_squared_error( real_y[max_timestep:], predicted_y[max_timestep:] ))
        PCC = pearsonr( real_y[max_timestep:] , predicted_y[max_timestep:] )
    ...
    my_prediction_1, Ground_Truth_1, my_prediction_2, Ground_Truth_2):
        ...
        if kinematic_variable_type == 'x_and_y_vel':
            ...
            df = pd.DataFrame( my_prediction_2 )
            df.to_csv(os.path.join(csv_path, 'my_prediction_y_vel.csv'), index=False, header=False)
