...
for k in range(len(session_file_list)):
    ...
    instance_phase_all_channels='Tables/'+ session_name +'/'+bandwidth_token+'/24kHz/instance_phase_all_channels.csv'
    ...
    iterator=enumerate(zip( pd.read_csv(new_nwb_time_stamp, chunksize=chunksize), pd.read_csv(instance_phase_all_channels, chunksize=chunksize)  ))
    ...
    for nwb_loop_index, (chunk_new_nwb_time_stamp, chunk_instantaneous_phase) in iterator:
        ...
        for mat_loop_index in range(skip, mat_timestamp.shape[1]):
            ...
            # ITPC_abs_250Hz=[]
            # ITPC_angle_250Hz=[]
            instance_phase_all_channels_250Hz=[]
            ...
            if chunk_index[0].size>0:
                ...
                instance_phase_all_channels_250Hz.append( chunk_instantaneous_phase[chunk_index[0][0]] )


                nwb_timestamp_to_mat_timestamp=np.array(nwb_timestamp_to_mat_timestamp).transpose()
                instance_phase_all_channels_250Hz=np.array(instance_phase_all_channels_250Hz).transpose()
                # Write result to csv
                CWD = this_cwd
                # CWD= os.path.join('..')
                ...
                if is_first_loop==True:
                    ...
                    try:
                        ...
                        os.remove(os.path.join(csv_path,'instance_phase_all_channels_250Hz.csv'))
                        print('\nOld file deleted\n')
                    except:
                        print('\nNo old files\n')
                ...
                df=pd.DataFrame(instance_phase_all_channels_250Hz)
                df.to_csv(os.path.join(csv_path,'instance_phase_all_channels_250Hz.csv'), mode='a', index=False, header=False)
    ...
    os.remove(instance_phase_all_channels)
