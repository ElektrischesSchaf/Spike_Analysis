    ...
    for channel_number_i in range(32):
        ...
        for nwb_loop_index, (chunk_new_nwb_time_stamp, chunk_instantaneous_phase_1, chunk_instantaneous_phase_2, chunk_instantaneous_phase_3) in iterator:
            ...
            for mat_loop_index in range(skip, mat_timestamp.shape[1]):
                ...
                instance_phase_a_channel_250Hz_1=[]
                ...
                if chunk_index[0].size>0:                    
                    ...
                    instance_phase_a_channel_250Hz_2.append( chunk_instantaneous_phase_2[chunk_index[0][0]] )
                    instance_phase_a_channel_250Hz_3.append( chunk_instantaneous_phase_3[chunk_index[0][0]] )

                    nwb_timestamp_to_mat_timestamp=np.array(nwb_timestamp_to_mat_timestamp).transpose()
                    instance_phase_a_channel_250Hz_1=np.array(instance_phase_a_channel_250Hz_1).transpose()
                    instance_phase_a_channel_250Hz_2=np.array(instance_phase_a_channel_250Hz_2).transpose()
                    instance_phase_a_channel_250Hz_3=np.array(instance_phase_a_channel_250Hz_3).transpose()
                    # Write result to csv
                    CWD = this_cwd
                    # CWD= os.path.join('..')
