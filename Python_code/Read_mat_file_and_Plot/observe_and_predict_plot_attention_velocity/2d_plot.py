...
position_reconstuction_path = os.path.join(CWD, 'velocity_reconstrution_vs_time')
if not os.path.exists(position_reconstuction_path):
    os.mkdir(position_reconstuction_path)

x_ground_truth = pd.read_csv("Ground_Truth_x_vel.csv", header=None)
x_ground_truth=np.array(x_ground_truth)

y_ground_truth = pd.read_csv("Ground_Truth_y_vel.csv", header=None)
...
prediction_x_attention = pd.read_csv("attention/my_prediction_x_vel.csv", header=None)
prediction_x_attention = np.array(prediction_x_attention)

prediction_y_attention = pd.read_csv("attention/my_prediction_y_vel.csv", header=None)
prediction_y_attention = np.array(prediction_y_attention)

prediction_x_no_attention = pd.read_csv("no_attention/my_prediction_x_vel.csv", header=None)
prediction_x_no_attention = np.array(prediction_x_no_attention)

prediction_y_no_attention = pd.read_csv("no_attention/my_prediction_y_vel.csv", header=None)
...
while(end_time_bin < int( prediction_y_no_attention.shape[0] )-duration ):
    ...
    plt.xlabel('Time (second)', fontsize=my_fontsize*0.5, color='black')
    plt.ylabel('Velocity (mm/s)', fontsize=my_fontsize*0.5, color='black')
    plt.xticks(fontsize=my_fontsize*0.5, color='black')
    plt.yticks(fontsize=my_fontsize*0.5, color='black')
    plt.legend(fontsize=my_fontsize*0.3, loc='upper right')
    plt.tight_layout()
