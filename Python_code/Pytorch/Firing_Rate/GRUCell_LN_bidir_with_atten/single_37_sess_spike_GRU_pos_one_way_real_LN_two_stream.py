...
from  Deep_Learning_Models.GRU_real_LN_self_Atten import Real_Layer_GRU_bidir
...
model_name = 'GRU_with_Spike_Single_49_Session_2_outputs_bidir_LN_two_stream'
MAX_EPOCH = 75
LEARNING_RATE = 1e-5
NUMBER_OF_LAYERS = 2
OUTPUT_DIM = 2
...
for session_k in range(len(session_file_list)):
    ...
    net = Real_Layer_GRU_bidir(input_dim = feature_numbers, hidden_dim = hidden_dim, max_timestep = max_timestep, layer_dim = layer_dim, output_dim = output_dim, sorted_unit_numbers=units_numbers_in_this_dataset)     # define the network    # print(net)  # net architecture
    ...
    def _run_iter(x,y):
        ...
        o_labels, attn_weight_matrix_forward_M1, attn_weight_matrix_forward_S1 = net(feature)
        torch.set_default_tensor_type('torch.cuda.FloatTensor')

        attn_weight_matrix_forward_M1 = attn_weight_matrix_forward_M1.to(device)        
        penality_loss_forward_M1 = torch.norm(  input = (torch.bmm(  attn_weight_matrix_forward_M1, torch.transpose(attn_weight_matrix_forward_M1, 1, 2) ) - torch.eye( attn_weight_matrix_forward_M1.size(1) )), p = 'fro')

        attn_weight_matrix_forward_S1 = attn_weight_matrix_forward_S1.to(device)        
        penality_loss_forward_S1 = torch.norm(  input = (torch.bmm(  attn_weight_matrix_forward_S1, torch.transpose(attn_weight_matrix_forward_S1, 1, 2) ) - torch.eye( attn_weight_matrix_forward_S1.size(1) )), p = 'fro')

        l_loss = loss_func(o_labels, labels) + penality_loss_forward_M1 + penality_loss_forward_S1
    ...
    for i, (x, testing_y) in trange:
        ...
        o_labels, attn_weight_matrix_forward_M1, attn_weight_matrix_forward_S1 = net(x.to(device))

        # attention map
        # attn_weight_matrix = attn_weight_matrix.squeeze(1)
        attn_weight_matrix = torch.cat(  (torch.sum(attn_weight_matrix_forward_M1, dim=1) , torch.sum(attn_weight_matrix_forward_S1, dim=1)), 1)
    ...
    while time_bin_index < (testing_data_length -plottin_duration_time_bin*2 ):
        Plotting.attention_map_2_outputs_2_stream_with_target_cue(session_name=session_name, type_name='pos', start_time_bin=time_bin_index, end_time_bin=time_bin_index+plottin_duration_time_bin, plot_path=attention_plot_path, my_prediction_1=my_prediction_1, Ground_Truth_1=Ground_Truth_1, my_prediction_2=my_prediction_2, Ground_Truth_2=Ground_Truth_2, attn_weight_matrix_forward_M1=attn_weight_matrix_all[:,:max_timestep], attn_weight_matrix_forward_S1=attn_weight_matrix_all[:,-max_timestep:], x_target_cue= x_target_all, y_target_cue=y_target_all, firing_rate_collector=firing_rate_collector)
